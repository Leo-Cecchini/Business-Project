...
import DocumentsList from '../components/azienda/DocumentsList';
        ...
        <div className="space-y-4">
            ...
            <Card>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-3 mb-4">
                    <KPICard
                        title="Operai"
                        value={overview?.stats?.workers_total ?? dashboard?.stats?.workers_total}
                        loading={isLoading}
                    />
                    <KPICard
                        title="Cantieri"
                        value={overview?.stats?.projects_total ?? dashboard?.stats?.projects_total}
                        loading={isLoading}
                    />
                    <KPICard
                        title="Cantieri attivi"
                        value={overview?.stats?.active_projects ?? dashboard?.stats?.active_projects}
                        loading={isLoading}
                    />
                    <KPICard
                        title="Operai attivi"
                        value={overview?.stats?.active_workers ?? dashboard?.stats?.active_workers}
                        loading={isLoading}
                    />
                    <KPICard
                        title="Documenti"
                        value={overview?.stats?.documents_total ?? dashboard?.stats?.documents_total}
                        loading={isLoading}
                    />
                </div>

                <div className="flex flex-wrap gap-2">
                    <Button onClick={() => setShowCreateWorker(true)}>
                        + Aggiungi operaio
                    </Button>
                    <Button variant="danger" onClick={() => setShowDeleteWorker(true)}>
                        − Rimuovi operaio
                    </Button>
