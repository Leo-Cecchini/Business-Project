...
#!/usr/bin/env python3
"""CLI wrapper for analytics service."""
...
def main():
    ...
    # Connect
    connect(db="business_project")
    db = get_db()
    
    if args.dry_run:
        # Just show metrics
        records = AnalyticsService.fetch_records(db, args.days, args.limit, args.site_id)
        ...
        # Generate and save
        report = AnalyticsService.generate_and_save_report(db, args.days, args.limit, args.site_id)
        print(f"✅ Report saved: {report.get('id')}")
